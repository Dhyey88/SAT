...
/// Enforces a strict 1-minute (60 seconds) background session timeout.
/// 
/// Rules:
/// 1. As long as the app is open / running in background for less than 1 minute, the session stays active.
/// 2. If the app remains in the background for more than 1 minute, the session expires automatically.
...
final class SessionManager {
    ...
    // MARK: - Configuration
    /// Session timeout interval: 1 minute (60 seconds)
    static let sessionTimeoutSeconds: TimeInterval = 60.0
    ...
    func recordBackgroundTransition() {
        ...
        print("[SessionManager] App entered background at \(lastBackgroundTimestamp!). Starting 1-minute timeout window.")
    }
    ...
    func checkSessionOnForeground() -> (isValid: Bool, reason: String?) {
        ...
        if elapsed > SessionManager.sessionTimeoutSeconds {
            let message = "Session expired after 1 minute of inactivity. Please log in again."
    ...
    func checkAndHandleSessionOnForeground(in window: UIWindow?) {
        ...
        if !result.isValid {
            // Session expired: smoothly transition root view to LoginViewController with warning
            DispatchQueue.main.async {
                guard let window = window else { return }
                let loginVC = LoginViewController(sessionExpiredMessage: result.reason)
