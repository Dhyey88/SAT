...
class SignUpViewController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    // MARK: - Error Handlers & Message Extraction Helper
    /// Prioritizes API-provided validation messages; uses static fallback if not provided by server
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            DispatchQueue.main.async {
                ...
                if status {
                    ...
                    let merchantId = (dataObj["id"] as? Int) ?? (dataObj["merchant_id"] as? Int) ?? 6
                    ...
                    self.trustErrorLabel.text = apiMessage
    ...
    @objc private func handleEmailSubmit() {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            DispatchQueue.main.async {
                ...
                let status = json["status"] as? Bool ?? false
                let apiMessage = self.extractMessage(from: json, fallback: "This Email id is already registered.")
                ...
                if status {
                    ...
                    self.emailErrorLabel.text = apiMessage
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            DispatchQueue.main.async {
                ...
                if status {
                    ...
                    self.mobileErrorLabel.text = apiMessage
    ...
    @objc private func handleParentCodeSubmit() {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            DispatchQueue.main.async {
                ...
                let status = json["status"] as? Bool ?? false
                let apiMessage = self.extractMessage(from: json, fallback: "Invalid Main/Parent Branch Code.")
                ...
                if status {
                    ...
                    self.parentErrorLabel.text = apiMessage
